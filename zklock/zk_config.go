...
func RegisterConfigHandler(namespace string, key string, action HandlerFunc) error {
	...
	if _, err := CreatePath(configShareBasePath); err != nil {
		return fmt.Errorf("create config base path failed %s", configShareBasePath)
	}
	ns := fmt.Sprintf("%s/%s", configShareBasePath, namespace)
	if _, err := CreatePath(ns); err != nil {
		return fmt.Errorf("create config namespace path failed %s", ns)
	}
	keyPath := fmt.Sprintf("%s/%s", ns, key)
	if _, err := CreatePath(keyPath); err != nil {
		return fmt.Errorf("create config key path failed %s", keyPath)
	}

	if data, _, ch, err := ZkClient.GetW(keyPath); err != nil {
		log.Errorf("Get config keyPath %s data failed with error : %s", keyPath, err.Error())
	} else {
		log.Infof("Get config keyPath %s data success", keyPath)
		action(data)
		go func() {
			for {
				select {
				case evt := <- ch:
					if evt.Type == zk.EventNodeDataChanged {
						if data, _, chx, err := ZkClient.GetW(keyPath); err != nil {
							...
							log.Infof("config keyPath %s data changed !!", keyPath)
							ch = chx
							action(data)
						}
					}
